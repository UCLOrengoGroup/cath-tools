...
str_opt cath::sup::get_invalid_description(const superposition_content_spec &arg_spec ///< The superposition_content_spec to query
                                           ) {
	if ( arg_spec.get_include_dna_within_distance() && arg_spec.get_include_dna_within_distance() < 0.0 ) {
		return
			"The "
			+ superposition_content_options_block::PO_INCLUDE_DNA_WITHIN_DISTANCE
			+ " distance cannot be negative";
	}
	if ( arg_spec.get_include_organic_within_distance() && arg_spec.get_include_organic_within_distance() < 0.0 ) {
		return
			"The "
