...
namespace cath {
	namespace rslv {
		namespace detail {
			...
			/// Build a calc_hit_vec of hits whilst pruning some (but not reliably all)
			...
			private:
				/// The calc_hit_vec to be built up
				calc_hit_vec hits;

				/// A lookup from a hash of the segments signature to an index of an example calc_hit
				/// with that segments signature
				std::unordered_map<size_t, size_t> index_of_signature_hash;

			public:
				/// Reserve space for the specified number of hits
				...
				/// Return the number of hits stored thus far (after any pruning)
				inline size_t size() const {
					return hits.size();
				}

				/// Whether there haven't been any hits built so far
				inline bool empty() const {
					return hits.empty();
				}

				/// Add a hit (or skip it if it's found to be worse than existing hit)
				...
				/// Get a non-const reference to the built hits
