...
/// \brief Get the specified boundary for the specified segment in the context of the other segments and trim_spec
...
res_arrow_opt cath::rslv::detail::get_boundary_impl(const boundary_wanted &arg_dirn,           ///< The direction of the boundary to query
	...
	// on the correct side of the query segment
	const auto is_on_correct_side_fn = [&] (const hit_seg &x) {
		return in_wanted_dirn_fn( x, arg_segment );
	};

	// If none of the other segments are on the correct side of the query, return none
	if ( none_of( other_segments, is_on_correct_side_fn ) ) {
		return none;
	}

	// Build a vector of the segments that are on the correct side
	//
	...
	const auto other_segs_on_correct_side = copy_build<hit_seg_vec>(
		other_segments | filtered( is_on_correct_side_fn )
	);
...
/// \brief Get the start boundary for the specified segment in the context of the other segments and trim_spec
...
/// \brief Get the stop boundary for the specified segment in the context of the other segments and trim_spec
...
/// \brief Get the start/stop boundary pair for the specified segment in the context of the other segments and trim_spec
seg_boundary_pair cath::rslv::get_boundary_pair(const hit_seg     &arg_segment,        ///< The hit to query
                                                const hit_seg_vec &arg_other_segments, ///< The other segments from other hits
                                                const trim_spec   &arg_trim_spec       ///< The trim_spec to use
                                                ) {
	return make_pair(
		get_start_boundary( arg_segment, arg_other_segments, arg_trim_spec ),
		get_stop_boundary ( arg_segment, arg_other_segments, arg_trim_spec )
	);
}
