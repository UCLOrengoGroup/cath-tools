...
/// \brief Calculate the starts and indices of the discontiguous domains in the specified hit_list,
///        ordered by their starts
res_arr_idx_pair_vec discont_hits_index_by_start::calc_disconts(const hit_list &arg_hit_list ///< The hit_list to index
                                                                ) {
	return sort_copy(
		copy_build<res_arr_idx_pair_vec>(
			irange( 0_z, arg_hit_list.size() )
				| filtered   ( [&] (const size_t &x) {
					return arg_hit_list[ x ].is_discontig();
				} )
				| transformed( [&] (const size_t &x) {
					return make_pair( arg_hit_list[ x ].get_start_arrow(), numeric_cast<hitidx_t>( x ) );
				} )
		)
	);
}

/// \brief Ctor from a hit_list
discont_hits_index_by_start::discont_hits_index_by_start(const hit_list &arg_hit_list ///< The hit_list to index
                                                         ) : the_hits ( arg_hit_list                   ),
                                                             disconts ( calc_disconts ( arg_hit_list ) ) {
}
...
/// \brief Return a reference to the hit associated with an index
///
/// The only source of sensible arguments to this method is get_index_indices_of_disconts_in_range()
const hit & discont_hits_index_by_start::get_discont_hit_of_index_index(const size_t &arg_index_index ///< The index (in this discont_hits_index_by_start) of the hit to return
                                                                        ) const {
