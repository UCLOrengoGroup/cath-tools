...
namespace cath::test {
	...
	class files_equal final {
	  ...
	  public:
		explicit files_equal( const bootstrap_mode &, const str_size_type & = istreams_equal::DEFAULT_DIFF_HALF_WIDTH );
		explicit files_equal( const str_size_type & = istreams_equal::DEFAULT_DIFF_HALF_WIDTH );

		boost::test_tools::predicate_result operator()( const ::std::filesystem::path &, const ::std::filesystem::path & ) const;
