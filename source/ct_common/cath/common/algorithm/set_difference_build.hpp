/// \file
/// \brief The set_difference_build() header
...
#ifndef _CATH_TOOLS_SOURCE_CT_COMMON_CATH_COMMON_ALGORITHM_SET_DIFFERENCE_BUILD_HPP
#define _CATH_TOOLS_SOURCE_CT_COMMON_CATH_COMMON_ALGORITHM_SET_DIFFERENCE_BUILD_HPP
...
namespace cath {
	namespace common {
		...
		///
		/// WARNING: set_difference() is a bit rubbish - it copies the first element of the input range into
		///          the output range before doing calculating the set_differences. In practice, this means
		///          the output range must have the same value type and the input range.
		///
		/// \sa set_difference_build() generate_n_build() random_sample_n_build() sort_set_difference() sort_uniq_set_difference() transform_build() uniq_set_difference()
		///
		...
		inline Container set_difference_build(const SinglePassRange1 &rng1, ///< A single-pass input range
			...
			// Call the normal Boost Range set_difference()
...
#endif // _CATH_TOOLS_SOURCE_CT_COMMON_CATH_COMMON_ALGORITHM_SET_DIFFERENCE_BUILD_HPP
