...
str_str_score_tpl string_aligner::align(const string      &prm_sequence_string_a, ///< The first  string to be aligned
                                        const string      &prm_sequence_string_b, ///< The second string to be aligned
	...
	const str_str_pair aligned_strings = do_align(
		prm_sequence_string_a,
		prm_sequence_string_b,
		prm_gap_penalty
	);

	// Perform some checks on the output
	check_aligned_string_pair_is_valid(    aligned_strings.first,  aligned_strings.second );
	check_aligned_string_matches_original( aligned_strings.first,  prm_sequence_string_a  );
	check_aligned_string_matches_original( aligned_strings.second, prm_sequence_string_b  );
...
void cath::align::detail::check_aligned_string_matches_original(const string &prm_aligned_string, ///< The aligned version of the original string
                                                                const string &prm_orig            ///< The original string that was to be aligned
                                                                ) {
	const string letters_from_aligned = find_format_all_copy( prm_aligned_string, token_finder( ! is_upper() ), empty_formatter(prm_aligned_string) );
	if ( prm_orig != letters_from_aligned ) {
...
void cath::align::detail::check_aligned_string_pair_is_valid(const string &prm_aligned_string_a, ///< The aligned version of the first string
                                                             const string &prm_aligned_string_b  ///< The aligned version of the second string
                                                             ) {
	// Check the two strings are of equal length
	if ( prm_aligned_string_a.length() != prm_aligned_string_b.length() ) {
		BOOST_THROW_EXCEPTION(invalid_argument_exception(
			"Supposedly aligned strings \""
			+ prm_aligned_string_a
			+ "\" and \""
			+ prm_aligned_string_b
			+ "\" are not of equal length"
		));
	}

	// Perform further checks on each string
	check_aligned_string_is_valid( prm_aligned_string_a );
	check_aligned_string_is_valid( prm_aligned_string_b );
}
...
void cath::align::detail::check_aligned_string_is_valid(const string &prm_aligned_string ///< An aligned version of a string
                                                        ) {
	// Create a trimmed copy with spaces removed from the end
	const string trimmed = trim_copy(prm_aligned_string);
...
size_size_pair cath::align::detail::get_num_gaps_and_extensions(const string &prm_aligned_string ///< The aligned string (must contain upper-case letters, possibly split by '-' characters and possibly with a head/tail of space characters)
                                                                ) {
...
score_type cath::align::detail::get_score_of_aligned_sequence_strings(const string      &prm_aligned_string_a, ///< The aligned version of the first string
                                                                      const string      &prm_aligned_string_b, ///< The aligned version of the second string
                                                                      const gap_penalty &prm_gap_penalty       ///< The gap penalty to be applied for each gap step (ie for opening OR extending a gap)
                                                                      ) {
	// Sanity check the input strings
	check_aligned_string_pair_is_valid( prm_aligned_string_a, prm_aligned_string_b );

	// Count the number of matching characters
	const string::size_type str_length = prm_aligned_string_a.length();
	size_t num_matches = 0;
	for (const string::size_type &char_ctr : indices( str_length ) ) {
		if ( prm_aligned_string_a[ char_ctr ] == prm_aligned_string_b[ char_ctr ] ) {
			++num_matches;
		}
	}
	...
	const size_size_pair gap_counts_a      = get_num_gaps_and_extensions( prm_aligned_string_a );
	const size_size_pair gap_counts_b      = get_num_gaps_and_extensions( prm_aligned_string_b );
	...
	return   ( numeric_cast<score_type>( num_matches       ) * score_per_match                          )
	       - ( numeric_cast<score_type>( total_gap_opens   ) * prm_gap_penalty.get_open_gap_penalty()   )
	       - ( numeric_cast<score_type>( total_gap_extends ) * prm_gap_penalty.get_extend_gap_penalty() );
}
...
//score_alignment_pair cath::align::detail::align_sequence_strings(const dyn_prog_aligner &prm_dyn_prog_aligner,  ///< TODOCUMENT
//                                                                 const string           &prm_sequence_string_a, ///< The first  sequence to align
//                                                                 const string           &prm_sequence_string_b, ///< The second sequence to align
//                                                                 const score_type       &prm_gap_penalty       ///< The gap penalty to be applied for each gap step (ie for opening OR extending a gap)
////                                                                 const size_t     &prm_window_width       ///< The width of the window (details? TODOCUMENT)
//                                                                 ) {
//	const gen_dyn_prog_string_aligner the_string_aligner(prm_dyn_prog_aligner);
//	const str_str_score_tpl bob = the_string_aligner.align(prm_sequence_string_a, prm_sequence_string_b, prm_gap_penalty);
...
///
/// \pre The alignment must contain the same number of entries as prm_sequence_strings.
///
...
///
/// \relates sequence_string_dyn_prog_score_source
str_vec cath::align::detail::format_alignment_strings(const alignment      &prm_alignment,       ///< The alignment to format
                                                      const str_vec &prm_sequence_strings ///< The strings corresponding to the alignment
                                                      ) {
	// Grab the dimensions of the inputs
	const alignment::size_type num_entries          = prm_alignment.num_entries();
	const alignment::size_type num_sequence_strings = prm_sequence_strings.size();
	...
	for (const size_t &alignment_entry : indices( num_entries ) ) {
		...
		const string            &sequence_string   = prm_sequence_strings[ alignment_entry ];
		...
		for (const size_t &alignment_idx : indices( alignment_length ) ) {
			...
			const aln_posn_opt position = prm_alignment.position_of_entry_of_index( alignment_entry, alignment_idx );
			if ( position ) {
...
///
/// \relates sequence_string_dyn_prog_score_source
str_str_pair cath::align::detail::format_alignment_strings(const alignment &prm_alignment,         ///< The alignment to format
                                                           const string    &prm_sequence_string_a, ///< The first  string corresponding to the alignment
                                                           const string    &prm_sequence_string_b  ///< The second string corresponding to the alignment
                                                           ) {
	const str_vec sequence_strings  = { prm_sequence_string_a,
	                                    prm_sequence_string_b };
...
/////
///// \relates sequence_string_dyn_prog_score_source
//str_str_pair cath::align::detail::align_and_format_sequence_strings(const string     &prm_sequence_string_a, ///< The first  sequence to align
//                                                                    const string     &prm_sequence_string_b, ///< The second sequence to align
//                                                                    const score_type &prm_gap_penalty,       ///< The gap penalty to be applied for each gap step (ie for opening OR extending a gap)
//                                                                    const size_t     &prm_window_width       ///< The width of the window (details? TODOCUMENT)
//                                                                    ) {
...
//                                                                    const score_type &prm_gap_penalty        ///< The gap penalty to be applied for each gap step (ie for opening OR extending a gap)
//                                                                    ) {
//	const size_t length_a   = prm_sequence_string_a.length();
//	const size_t length_b   = prm_sequence_string_b.length();
