...
/// \brief TODOCUMENT
score_type mask_dyn_prog_score_source::do_get_score(const size_t &prm_index_a, ///< The index of the element of interest in the first  sequence
	...
	const bool mask_result = mask_matrix.get( prm_index_b + 1, prm_index_a + 1 );

	// If the mask value is true then return the masked_score_source's score, else return 0
	return mask_result ? masked_score_source.get_score(prm_index_a, prm_index_b)
	                   : 0;
}

/// \brief Ctor for mask_dyn_prog_score_source
mask_dyn_prog_score_source::mask_dyn_prog_score_source(const bool_vec_of_vec       &prm_mask_matrix,        ///< TODOCUMENT
                                                       const dyn_prog_score_source &prm_masked_score_source ///< TODOCUMENT
                                                       ) : mask_matrix        ( prm_mask_matrix         ),
                                                           masked_score_source( prm_masked_score_source ) {
}
