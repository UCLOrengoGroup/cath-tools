...
void viewer::do_end_colouring(ostream      &/*prm_ostream*/, ///< The ostream to which the PyMOL commands should be written
                              const string &/*prm_colouring_label*/ ///< The label to be used for the colouring that is ending
                              ) {
}
...
void viewer::write_end(ostream          &prm_os,            ///< TODOCUMENT
	...
	do_write_end( prm_os, prm_advert_message );
}
...
void cath::output_superposition_to_viewer(ostream                          &prm_ostream,               ///< The ostream to which the data should be written
	...
	if ( missing_wanted_alignment || would_accept_extra_consecutive) {
		if ( missing_wanted_alignment ) {
			constexpr auto message = "Unable to apply an alignment-based colouring scheme to the superposition because it doesn't contain an alignment";
	...
	// Write the start of the viewer output
	const string advert_msg = "Superposition generated by cath-superpose (" + cath_tools_git_version() + "), one of the cath-tools.";

	prm_viewer.write_end( prm_ostream, advert_msg );
}
