...
namespace {
	...
	protected:
	  ...
	  public:
		...
		static constexpr ::std::string_view id1 = "1a04A02";

		/// \brief A string containing the ID for 1fseB00 to be used in a ssap_pair_fixture
		static constexpr ::std::string_view id2 = "1fseB00";

		stringstream_log_sink log_sink;
		const data_dirs_spec data_dirs = build_data_dirs_spec_of_dir( TEST_SSAP_REGRESSION_DATA_DIR() );
		const prot_prot_pair parsed_proteins =
		  read_protein_pair( string( id1 ), nullopt, string( id2 ), nullopt, data_dirs, protein_from_wolf_and_sec(), nullopt );
	...
	/// \brief TODOCUMENT
	void ssap_test_suite_fixture::check_context_sec_scores_as_expected() const {
	...
	//	cerr << endl << endl;
		const vector<score_type> expected_context_sec_scores = read_file<score_type>(
			TEST_SSAP_REGRESSION_DATA_DIR() / ::fmt::format( "{}_{}.expected_context_sec_scores", id1, id2 )
		);
		BOOST_CHECK_EQUAL_RANGES( expected_context_sec_scores, got_context_sec_scores );
	}

	/// \brief TODOCUMENT
	void ssap_test_suite_fixture::check_residues_have_similar_area_angle_props() const {
	...
	//	);
		...
		const vector<bool> expected_residues_similar = read_file<bool>(
			TEST_SSAP_REGRESSION_DATA_DIR() / ::fmt::format( "{}_{}.expected_residues_similar", id1, id2 )
		);
...
BOOST_FIXTURE_TEST_SUITE(ssap_test_suite, ssap_test_suite_fixture)
...
BOOST_AUTO_TEST_CASE(check_globals_are_being_reset_before_tests_1) {
...
BOOST_AUTO_TEST_CASE(residues_have_similar_area_angle_props_1a04A02_1fseB00) {
