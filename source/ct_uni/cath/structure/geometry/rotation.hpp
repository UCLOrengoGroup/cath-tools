...
namespace cath {
	...
	//		doub_vec_vec matrix;
		...
		inline void rotation::check_index(const size_t &prm_index ///< TODOCUMENT
		                                  ) {
	#ifndef NDEBUG
		...
		/// \brief Check that a dimension index is a sensible value
		///        (or else throw an exception)
		inline void rotation::check_value(const double &prm_value
		                                  ) {
	#ifndef NDEBUG
			if ( ! boost::math::isfinite( prm_value ) ) {
				BOOST_THROW_EXCEPTION(cath::common::invalid_argument_exception("Value is not a sensible finite number"));
			}
	#else
			boost::ignore_unused( prm_value );
	#endif
		}
		...
		/// \brief Private initialisation from row-major order vector to be used by multiple ctors
		///        and then basic checks that this is a valid rotation
		inline rotation & rotation::init_from_values_with_rotation_checks(const double &prm_val_00, const double &prm_val_01, const double &prm_val_02,
			...
			init_from_values( prm_val_00, prm_val_01, prm_val_02,
			                  prm_val_10, prm_val_11, prm_val_12,
			                  prm_val_20, prm_val_21, prm_val_22 );
			check_is_valid_rotation();
			return *this;
		}

		/// \brief Private initialisation from row-major order vector to be used by multiple ctors
		inline rotation & rotation::init_from_values(const double &prm_val_00, const double &prm_val_01, const double &prm_val_02,
		                                             const double &prm_val_10, const double &prm_val_11, const double &prm_val_12,
		                                             const double &prm_val_20, const double &prm_val_21, const double &prm_val_22
		                                             ) {
		...
		inline rotation & rotation::set_value(const size_t &prm_row_index, ///< TODOCUMENT
		...
		/// \brief TODOCUMENT
		template <size_t row_index, size_t col_index>
		inline void rotation::set_value(const double &prm_value ///< TODOCUMENT
		                                ) {
		...
		inline const double & rotation::get_value(const size_t &prm_row_index, ///< TODOCUMENT
		                                          const size_t &prm_col_index  ///< TODOCUMENT
		                                          ) const {
			check_index( prm_row_index );
			check_index( prm_col_index );
		...
		inline void rotation::operator*=(const rotation &prm_rotation ///< TODOCUMENT
		                                 ) {
		...
		/// \relates rotation
		template<size_t row_index>
		inline coord get_row(const rotation &prm_rotation ///< TODOCUMENT
		                     ) {
			return coord(
		...
		/// \relates rotation
		template<size_t col_index>
		inline coord get_column(const rotation &prm_rotation ///< TODOCUMENT
		                        ) {
			return coord(
		...
		inline rotation transpose_copy(const rotation &prm_rotation ///< TODOCUMENT
		                               ) {
			return rotation(
		...
		///
		/// \relates rotation
		inline rotation rotation_between_rotations(const rotation &prm_rotation_1, ///< The first rotation
		...
		///
		/// \relates rotation
		inline double trace(const rotation &prm_rotation ///< TODOCUMENT
		                    ) {
...
//			std::cerr << "prm_rotation_2 : " << prm_rotation_2 << std::endl;
		...
		///
		/// \relates rotation
		inline coord rotate_copy(const rotation &prm_rotation, ///< TODOCUMENT
...
#endif // _CATH_TOOLS_SOURCE_CT_UNI_CATH_STRUCTURE_GEOMETRY_ROTATION_HPP
