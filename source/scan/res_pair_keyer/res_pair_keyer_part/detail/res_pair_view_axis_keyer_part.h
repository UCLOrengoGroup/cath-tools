/// \file
/// \brief The res_pair_view_axis_keyer_part class header
...
namespace cath {
	namespace scan {
		namespace detail {
			...
			template <typename F>
			class res_pair_view_axis_keyer_part final {
			public:
				/// \brief TODOCUMENT
			...
			public:
				...
				res_pair_view_axis_keyer_part(const value_t &arg_cell_width ///< The cell width to use in keying this part
				                              ) : cell_width( arg_cell_width ) {
					F::sanity_check_cell_width( cell_width );
				}

				/// \brief Get a short name that describes this key part
				std::string get_name() const {
					return F::get_name() + "(" + std::to_string( cell_width ) + "]";
				}
