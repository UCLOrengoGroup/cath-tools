...
#ifndef CATH_TOOLS_SOURCE_CT_RESOLVE_HITS_CATH_RESOLVE_HITS_FILE_DETAIL_HMMER_PARSER_HPP
#define CATH_TOOLS_SOURCE_CT_RESOLVE_HITS_CATH_RESOLVE_HITS_FILE_DETAIL_HMMER_PARSER_HPP
			...
			while ( getline( the_istream.get(), line ) && !line_is_at_aln() ) {
				if ( line_is_at_block() || end_of_istream() ) {
					BOOST_THROW_EXCEPTION( common::runtime_error_exception(
					  ...
					  + ( static_cast<bool>( query_id ) ? ( ", query: " + *query_id ) : "" )
					  + ( static_cast<bool>( prefix_match_id ) ? ( ", match: " + *prefix_match_id ) : "" )
...
#endif // CATH_TOOLS_SOURCE_CT_RESOLVE_HITS_CATH_RESOLVE_HITS_FILE_DETAIL_HMMER_PARSER_HPP
