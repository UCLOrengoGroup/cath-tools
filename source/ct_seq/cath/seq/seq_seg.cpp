...
///
/// \relates seq_seg
seq_seg_vec cath::seq::get_present_segments(const seq_seg_opt_vec &prm_segments ///< The vector of optional seq_segs to query
                                            ) {
...
///
/// \relates seq_seg
string cath::seq::get_segments_string(const seq_seg_vec &prm_segments ///< The segments to describe
                                      ) {
	return join(
		prm_segments
...
///
/// \relates seq_seg
seq_seg_vec cath::seq::make_fragments_of_segments(seq_seg_vec prm_segments ///< The segments from which the fragments should be made
                                                  ) {
	start_sort_seq_segs( prm_segments );
	return make_fragments_of_start_sorted_segments( prm_segments );
}

/// \brief Return whether the specified segments are sorted by their starts and non-overlapping
///
/// \relates seq_seg
bool cath::seq::segments_are_start_sorted_and_non_overlapping(const seq_seg_vec &prm_segments ///< The segments to query
                                                              ) {
...
///
/// \relates seq_seg
seq_seg_vec cath::seq::make_fragments_of_start_sorted_segments(const seq_seg_vec &prm_segments ///< The segments from which the fragments must be made, pre-sorted by their starts
                                                               ) {
	if ( prm_segments.empty() ) {
...
///
/// \relates seq_seg
bool cath::seq::midpoint_less(const seq_seg &prm_seq_seg_lhs, ///< The first  seq_seg to compare
                              const seq_seg &prm_seq_seg_rhs  ///< The second seq_seg to compare
                              ) {
	const auto midpoint_x2_fn = [] (const seq_seg &x) {
		return x.get_start_arrow().get_index() + x.get_stop_arrow().get_index();
	};
	return midpoint_x2_fn( prm_seq_seg_lhs ) < midpoint_x2_fn( prm_seq_seg_rhs );
}

/// \brief Generate a string describing the specified seq_seg
///
/// \relates seq_seg
string cath::seq::to_simple_string(const seq_seg &prm_seq_seg ///< The seq_seg to describe
                                   ) {
	return
		  ::std::to_string( get_start_res_index( prm_seq_seg ) )
		+ "-"
		+ ::std::to_string( get_stop_res_index ( prm_seq_seg ) );
}

/// \brief Generate a string describing the specified seq_seg_opt
///
/// \relates seq_seg_opt
string cath::seq::to_simple_string(const seq_seg_opt &prm_seq_seg_opt ///< The seq_seg_opt to describe
                                   ) {
	return prm_seq_seg_opt
		? to_simple_string( *prm_seq_seg_opt )
		: "absent";
}
...
///
/// \relates seq_seg
string cath::seq::to_simple_seg_string(const seq_arrow &prm_start_arrow, ///< The start position
                                       const seq_arrow &prm_stop_arrow   ///< The stop  position
                                       ) {
	return to_simple_string( seq_seg{ prm_start_arrow, prm_stop_arrow } );
}

/// \brief Generate a string describing the specified seq_seg
///
/// \relates seq_seg
string cath::seq::to_string(const seq_seg &prm_seq_seg ///< The seq_seg to describe
                            ) {
	return "seq_seg[" + to_simple_string( prm_seq_seg ) + "]";
}

/// \brief Insert a description of the specified seq_seg into the specified ostream
///
/// \relates seq_seg
ostream & cath::seq::operator<<(ostream       &prm_ostream, ///< The ostream into which the description of the seq_seg should be inserted
                                const seq_seg &prm_seq_seg  ///< The seq_seg to describe
                                ) {
	prm_ostream << to_string( prm_seq_seg );
	return prm_ostream;
}
