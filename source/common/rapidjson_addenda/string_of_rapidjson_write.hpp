...
namespace cath {
	namespace common {
		...
		std::string string_of_rapidjson_write(Fn           &&arg_fn,             ///< The write to perform on the writer (must be invokable with a single `rapidjson_writer<Style>&` argument)
			...
			// Find the end of the any preceding arg_extra_depth array-open chars
			// and the start of any following arg_extra_depth array-close chars
			const auto end_of_preceding   = ( arg_extra_depth > 0 )
			                                ? common::cend  ( boost::algorithm::find_nth( string_range, "[",     debug_numeric_cast<int>( arg_extra_depth - 1 ) ) )
			                                : c_string;
			const auto begin_of_following = ( arg_extra_depth > 0 )
			                                ? common::cbegin( boost::algorithm::find_nth( string_range, "]", 0 - debug_numeric_cast<int>( arg_extra_depth     ) ) )
			                                : c_string_end;
