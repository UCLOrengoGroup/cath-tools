...
/// \brief TODOCUMENT
void pymol_viewer::do_colour_pdb(ostream      &arg_os,          ///< TODOCUMENT
                                 const string &arg_colour_name, ///< TODOCUMENT
...
void pymol_viewer::do_colour_pdb_residues(ostream                &arg_os,          ///< TODOCUMENT
                                          const string           &arg_colour_name, ///< TODOCUMENT
                                          const string           &arg_pdb_name,    ///< TODOCUMENT
                                          const residue_name_vec &residue_names    ///< TODOCUMENT
                                          ) const {
	arg_os << "colour " << arg_colour_name << ",";

	const size_t num_res_names   = residue_names.size();
	...
	for (size_t batch_ctr = 0; batch_ctr < num_res_batches; ++batch_ctr) {
		...
		for (size_t res_index = begin_and_end.first; res_index < begin_and_end.second; ++res_index) {
			arg_os << ( res_index > 0 ? "+" : "" );
			arg_os << residue_names[ res_index ];
		}
		arg_os << "//";
	}
	arg_os << "\n";
