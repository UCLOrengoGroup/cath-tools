...
/*>int ReadRawPIR(FILE *fp, char **seqs, int maxchain, BOOL upcase,
   ...
   The only requirements of the code are that the PIR file should have
   2 title lines per entry, the first line starting with a > sign.
   ...
   *error = FALSE;
   ...
   for(;;)
   {
      ...
      /* Read characters, storing sequence and handling any 
         special punctuation (end or start of new sequence)
      */
      while((ch = fgetc(fp)) != EOF && ch != '*' && ch != '>')
      {
         ...
         else if(ch != ' ' && ch != '\t')
         {
            ...
            seqs[nseq][SeqPos++] = 
               (upcase ? (isupper(ch) ? ch : toupper(ch)) : ch);
