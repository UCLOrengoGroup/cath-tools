...
**************************************************************************
   ...
   V3.0  12.03.06 ReadWholePDB() and ReadWholePDBAtoms() are now included
                  as part of the main ReadPDB() package. This removes
                  the need for rewind()s
...
/*>PDB *doReadPDB(FILE *fp, int *natom, BOOL AllAtoms, int OccRank,
   ...
   12.03.07 V3.0  Rewritten as wrapper to doReadWholePDB()
*/
...
{
   WHOLEPDB *wpdb;
   if((wpdb = doReadWholePDB(fpin, natom, AllAtoms, OccRank, ModelNum,
                             FALSE))!=NULL)
   {
      return(wpdb->pdb);
   }
...
/*>WHOLEPDB *ReadWholePDB(FILE *fpin)
   ...
   12.03.07 Upated for new doReadWholePDB()
*/
WHOLEPDB *ReadWholePDB(FILE *fpin)
{
   WHOLEPDB *wpdb;
   ...
   wpdb = doReadWholePDB(fpin, &natoms, TRUE, 1, 1, TRUE);
   wpdb->pdb = RemoveAlternates(wpdb->pdb);
   return(wpdb);
}
...
WHOLEPDB *ReadWholePDBAtoms(FILE *fpin)
{
   int natoms;
   WHOLEPDB *wpdb;
   wpdb = doReadWholePDB(fpin, &natoms, FALSE, 1, 1, TRUE);
   wpdb->pdb = RemoveAlternates(wpdb->pdb);
   return(wpdb);
}
